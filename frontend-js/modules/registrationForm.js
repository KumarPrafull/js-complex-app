...
export default class RegistrationForm {
  ...
  // username field related function
  ...
  usernameImmediately(){
    if(this.username.value != "" && !/^([a-zA-Z0-9]+)$/.test(this.username.value)){
      this.showValidationError(this.username, "Your name can only contain letters and numbers.")
    }
  ...
  // email fields related functions
  ...
  emailDelay(){
    ...
    if(!this.email.errors){
      axios.post('/doesEmailExist', {_csrf: this._csrf, email: this.email.value}).then((response)=>{
        if (response.data) {
          this.email.isUnique = false
          this.showValidationError(this.email, "That email already exists.")
